...
def setup(on_startup):
    u'''Setup mining service internal environment.
    ...
    *before* you call setup() in the launcher script.'''
    ...
    log.info(u"Connecting to litecoind...")
    while True:
        try:
            result = (yield bitcoin_rpc.check_submitblock())
            if result:
                log.info(u"Found submitblock")
            elif result == False:
                log.info(u"Did not find submitblock")
            else:
                log.info(u"unknown submitblock result")
        except ConnectionRefusedError, e:
            log.error(
                u"Connection refused while trying to connect to the coind (are your COIND_* settings correct?)")
            reactor.stop()
            break
        ...
        try:
            ...
            if isinstance(result, dict):
                ...
                if result[u'version'] >= 1:
                    result = (yield bitcoin_rpc.getdifficulty())
                    if isinstance(result, dict):
                        if u'proof-of-stake' in result:
                            settings.COINDAEMON_Reward = u'POS'
                            log.info(u"Coin detected as POS")
                            break
                    else:
                        settings.COINDAEMON_Reward = u'POW'
                        log.info(u"Coin detected as POW")
                        break
                else:
                    log.error(u"Block Version mismatch: %s" % result[u'version'])
            ...
            if isinstance(e[2], unicode):
                try:
                    if isinstance(json.loads(e[2])[u'error'][u'message'], unicode):
                        error = json.loads(e[2])[u'error'][u'message']
                    if error == u"Method not found":
                        log.error(
                            u"CoinD does not support getblocktemplate!!! (time to upgrade.)")
                        reactor.stop()
                    elif u"downloading blocks" in error:
                        log.error(
                            u"CoinD downloading blockchain... will check back in 30 sec")
                        time.sleep(29)
                    else:
                        log.error(u"Coind Error: %s", error)
                except ValueError:
                    log.error(
                        u"Failed Connect(HTTP 500 or Invalid JSON), Check Username and Password!")
                    reactor.stop()
        time.sleep(1)  # If we didn't get a result or the connect failed

    log.info(u'Connected to the coind - Begining to load Address and Module Checks!')

    # Start the coinbaser
    coinbaser = SimpleCoinbaser(
        bitcoin_rpc, getattr(
            settings, u'CENTRAL_WALLET'))
    (yield coinbaser.on_load)

    registry = TemplateRegistry(BlockTemplate,
                                coinbaser,
                                bitcoin_rpc,
                                getattr(settings, u'INSTANCE_ID'),
    ...
    prune_thr = threading.Thread(
        target=WorkLogPruner, args=(
            Interfaces.worker_manager.job_log,))
    prune_thr.daemon = True
    prune_thr.start()

    log.info(u"MINING SERVICE IS READY")
