...
import lib.logger
log = lib.logger.get_logger(u'BasicShareLimiter')
...
u''' This is just a customized ring buffer '''


from __future__ import division
from __future__ import absolute_import
class SpeedBuffer(object):
...
class SpeedBufferFull(object):

    def __init__(self, n):
        raise u"you should use SpeedBuffer"
...
class BasicShareLimiter(object):
    ...
    def __init__(self):
        ...
        self.variance = self.target * \
            (float(settings.VDIFF_VARIANCE_PERCENT) / float(100))
    ...
    def update_litecoin_difficulty(self):
        ...
        if u'timestamp' not in self.litecoin or self.litecoin[
                u'timestamp'] < int(time.time()) - settings.DIFF_UPDATE_FREQUENCY:
            self.litecoin[u'timestamp'] = time.time()
            self.litecoin[u'difficulty'] = (yield Interfaces.template_registry.bitcoin_rpc.getdifficulty())
            log.debug(
                u"Updated litecoin difficulty to %s" %
                (self.litecoin[u'difficulty']))
        self.litecoin_diff = self.litecoin[u'difficulty']
    ...
    def submit(
        ...
        if worker_name not in self.worker_stats or self.worker_stats[
                worker_name][u'last_ts'] < ts - settings.DB_USERCACHE_TIME:
            self.worker_stats[worker_name] = {
                u'last_rtc': (
                    ts - self.retarget / 2),
                u'last_ts': ts,
                u'buffer': SpeedBuffer(
                    self.buffersize)}
            dbi.update_worker_diff(worker_name, settings.POOL_TARGET)
            return

        # Standard share update of data
        self.worker_stats[worker_name][u'buffer'].append(
            ts - self.worker_stats[worker_name][u'last_ts'])
        self.worker_stats[worker_name][u'last_ts'] = ts

        # Do We retarget? If not, we're done.
        if ts - self.worker_stats[worker_name][
                u'last_rtc'] < self.retarget and self.worker_stats[worker_name][u'buffer'].size() > 0:
            return

        # Set up and log our check
        self.worker_stats[worker_name][u'last_rtc'] = ts
        avg = self.worker_stats[worker_name][u'buffer'].avg()
        log.debug(u"Checking Retarget for %s (%i) avg. %i target %i+-%i" %
                  (worker_name, current_difficulty, avg, self.target, self.variance))

        if avg < 1:
            log.warning(u"Reseting avg = 1 since it's SOOO low")
            avg = 1

        # Figure out our Delta-Diff
        if settings.VDIFF_FLOAT:
            ddiff = float((float(current_difficulty) * \
                          (float(self.target) / float(avg))) - current_difficulty)
        else:
            ddiff = int((float(current_difficulty) * \
                        (float(self.target) / float(avg))) - current_difficulty)
        ...
        if (avg > self.tmax and current_difficulty > settings.VDIFF_MIN_TARGET):
            ...
            if settings.VDIFF_X2_TYPE:
                ...
                if settings.USE_COINDAEMON_DIFF:
                    ...
                    diff_max = min(
                        [settings.VDIFF_MAX_TARGET, self.litecoin_diff])
                else:
        ...
        if settings.VDIFF_X2_TYPE:
            ...
            new_diff = current_difficulty + ddiff
        log.debug(u"Retarget for %s %i old: %i new: %i" %
                  (worker_name, ddiff, current_difficulty, new_diff))

        self.worker_stats[worker_name][u'buffer'].clear()
        ...
        work_id = Interfaces.worker_manager.register_work(
            worker_name, job_id, new_diff)

        session[u'difficulty'] = new_diff
        connection_ref().rpc(
            u'mining.set_difficulty', [
                new_diff, ], is_notification=True)
        log.debug(u"Notified of New Difficulty")
        connection_ref().rpc(u'mining.notify',
                             [work_id,
                              prevhash,
                              coinb1,
                              coinb2,
