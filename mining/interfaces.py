u'''This module contains classes used by pool core to interact with the rest of the pool.
...
import lib.logger
log = lib.logger.get_logger(u'interfaces')
...
class WorkerManagerInterface(object):
    ...
    def __init__(self):
        self.worker_log = {}
        self.worker_log.setdefault(u'authorized', {})
        self.job_log = {}
        self.job_log.setdefault(u'None', {})
        return

    def authorize(self, worker_name, worker_password):
        # Important NOTE: This is called on EVERY submitted share. So you'll
        # need caching!!!
...
class ShareLimiterInterface(object):
    u'''Implement difficulty adjustments here'''
    ...
    def submit(
            ...
            timestamp,
            worker_name):
        u'''connection - weak reference to Protocol instance
...
class ShareManagerInterface(object):
    ...
    def on_network_block(self, prevhash, block_height):
        u'''Prints when there's new block coming from the network (possibly new round)'''
    ...
    def on_submit_share(
        ...
        log.debug(
            ...
            (block_hash,
             share_diff,
             u'valid' if is_valid else u'INVALID',
             worker_name))
        dbi.queue_share([worker_name,
                         block_header,
                         block_hash,
                         difficulty,
                         timestamp,
    ...
    def on_submit_block(
        ...
        log.info(
            u"Block %s %s" %
            (block_hash, u'ACCEPTED' if is_accepted else u'REJECTED'))
        dbi.do_import(dbi, True)
        dbi.found_block([worker_name, block_header, block_hash, -1, timestamp,
                         is_accepted, ip, self.block_height, self.prev_hash, share_diff])


class TimestamperInterface(object):
    u'''This is the only source for current time in the application.
...
class PredictableTimestamperInterface(TimestamperInterface):
    u'''Predictable timestamper may be useful for unit testing.'''
