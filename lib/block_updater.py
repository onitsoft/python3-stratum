...
import lib.logger
log = lib.logger.get_logger(u'block_updater')
...
class BlockUpdater(object):
    ...
    def __init__(self, registry, bitcoin_rpc):
        log.debug(u"Got To Block Updater")
    ...
    def schedule(self):
        ...
        log.debug(u"Next prevhash update in %.03f sec" % when)
        log.debug(
            u"Merkle update in next %.03f sec" %
            ((self.registry.last_update +
              settings.MERKLE_REFRESH_INTERVAL) -
             Interfaces.timestamper.time()))
        self.clock = reactor.callLater(when, self.run)

    def _get_next_time(self):
        when = settings.PREVHASH_REFRESH_INTERVAL - \
    ...
    def run(self):
        ...
        try:
            ...
            log.exception(u"UpdateWatchdog.run failed")
        finally:
            self.schedule()
