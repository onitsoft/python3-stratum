...
class CoinbaseTransactionPOW(halfnode.CTransaction):
    ...
    def __init__(self, timestamper, coinbaser, value, flags, height, data):
        ...
        log.debug(u"Got to CoinBaseTX")
        #self.extranonce = 0

        if len(self.extranonce_placeholder) != self.extranonce_size:
            raise Exception(
                u"Extranonce placeholder don't match expected length!")
        ...
        if settings.COINDAEMON_TX == u'yes':
            self.strTxComment = u"http://github.com/ahmedbodi/stratum-mining"
        self.vin.append(tx_in)
        self.vout.append(tx_out)

        # Two parts of serialized coinbase, just put part1 + extranonce + part2
        # to have final serialized tx
        self._serialized = super(
            CoinbaseTransactionPOW,
            self).serialize().split(
            self.extranonce_placeholder)

    def set_extranonce(self, extranonce):
        if len(extranonce) != self.extranonce_size:
            raise Exception(u"Incorrect extranonce size")
...
class CoinbaseTransactionPOS(halfnode.CTransaction):
    ...
    def __init__(
        ...
        self.nTime = ntime
        if settings.COINDAEMON_SHA256_TX == u'yes':
            self.strTxComment = u"http://github.com/ahmedbodi/stratum-mining"
        self.vin.append(tx_in)
        self.vout.append(tx_out)
