...
CENTRAL_WALLET = u'set_valid_addresss_in_config!'

COINDAEMON_TRUSTED_HOST = u'localhost'
COINDAEMON_TRUSTED_PORT = 28332
COINDAEMON_TRUSTED_USER = u'user'
COINDAEMON_TRUSTED_PASSWORD = u'somepassword'
...
# ******************** GENERAL SETTINGS ***************
# Set process name of twistd, much more comfortable if you run multiple
# processes on one machine
STRATUM_MINING_PROCESS_NAME = u'twistd-stratum-mining'
...
PASSWORD_SALT = u'some_crazy_string'

# ******************** Database  *********************
DATABASE_DRIVER = u'mysql'       # Options: none, sqlite, postgresql or mysql
DATABASE_EXTEND = False         # SQLite and PGSQL Only!

# SQLite
DB_SQLITE_FILE = u'pooldb.sqlite'
# Postgresql
DB_PGSQL_HOST = u'localhost'
DB_PGSQL_DBNAME = u'pooldb'
DB_PGSQL_USER = u'pooldb'
DB_PGSQL_PASS = u'**empty**'
DB_PGSQL_SCHEMA = u'public'
# MySQL
DB_MYSQL_HOST = u'localhost'
DB_MYSQL_DBNAME = u'pooldb'
DB_MYSQL_USER = u'pooldb'
DB_MYSQL_PASS = u'**empty**'
DB_MYSQL_PORT = 3306            # Default port for MySQL

# ******************** Adv. DB Settings *********************
#  Don't change these unless you know what you are doing

# How often we check to see if we should run the loader
DB_LOADER_CHECKTIME = 15
...
# How often the cache should be flushed into the DB regardless of size.
DB_LOADER_FORCE_TIME = 300
# When using the DATABASE_EXTEND option, average speed over X sec
DB_STATS_AVG_TIME = 300
...
# User Auth Options
# Automatically add users to database when they connect.
USERS_AUTOADD = False
# This basically disables User Auth for the pool.
# Check the workers password? (Many pools don't)
USERS_CHECK_PASSWORD = False

# Transaction Settings
# Extra Descriptive String to incorporate in solved blocks
COINBASE_EXTRAS = u'/stratumPool/'
...
# VARDIFF will start at the POOL_TARGET. It can go as low as the VDIFF_MIN
# and as high as min(VDIFF_MAX or coindaemons difficulty)
# Set the maximum difficulty to the coindaemon difficulty.
USE_COINDAEMON_DIFF = False
# Update the coindaemon difficulty once a day for the VARDIFF maximum
...
# Allow average time to very this % from target without retarget
...
# For People using the MPOS frontend enabling this is recommended. It
# allows the frontend to compare the block hash to the coin daemon
# reducing the likelihood of missing share error's for blocks
# If enabled, enter the block hash. If false enter the scrypt/sha hash
# into the shares table
SOLUTION_BLOCK_HASH = True

# Pass scrypt hash to submit block check.
# Use if submit block is returning errors and marking submitted blocks
# invalid upstream, but the submitted blocks are being a accepted by the
# coin daemon into the block chain.
BLOCK_CHECK_SCRYPT_HASH = False

# ******************** Worker Ban Options *********************
ENABLE_WORKER_BANNING = True    # Enable/disable temporary worker banning
# How long the worker stats cache is good before we check and refresh
WORKER_CACHE_TIME = 600
WORKER_BAN_TIME = 300           # How long we temporarily ban worker
# Allow average invalid shares vary this % before we ban
INVALID_SHARES_PERCENT = 50

# ******************** E-Mail Notification Settings *********************
# Where to send Start/Found block notifications
NOTIFY_EMAIL_TO = u''
# Where to send dead miner notifications
NOTIFY_EMAIL_TO_DEADMINER = u''
NOTIFY_EMAIL_FROM = u'root@localhost'                            # Sender address
NOTIFY_EMAIL_SERVER = u'localhost'                               # E-Mail sender
# E-Mail server SMTP logon
NOTIFY_EMAIL_USERNAME = u''
NOTIFY_EMAIL_PASSWORD = u''
NOTIFY_EMAIL_USETLS = True

# ******************** Memcache Settings *********************
# Memcahce is a requirement. Enter the settings below
MEMCACHE_HOST = u"localhost"     # Hostname or IP that runs memcached
MEMCACHE_PORT = 11211           # Port
MEMCACHE_TIMEOUT = 900          # Key timeout
MEMCACHE_PREFIX = u"stratum_"    # Prefix for keys
