...
u"""Pure Python implementation of the Skein 512-bit hashing algorithm"""
...
empty_bytes = array.array(u'B').tostring()


class Skein512(object):
    u"""Skein 512-bit hashing algorithm
    ...
    block_bits = 512
    block_type = {u'key': 0,
                  u'nonce': 0x5400000000000000,
                  u'msg': 0x7000000000000000,
                  u'cfg_final': 0xc400000000000000,
                  u'out_final': 0xff00000000000000}

    def __init__(self, msg=u'', digest_bits=512, key=None,
                 block_type=u'msg'):
        self.tf = Threefish512()
        if key:
            self.digest_bits = 512
            self._start_new_type(u'key')
            self.update(key)
            self.tf.key = bytes2words(self.final(False))
        self.digest_bits = digest_bits
        self.digest_size = (digest_bits + 7) >> 3
        self._start_new_type(u'cfg_final')
    ...
    def _start_new_type(self, block_type):
        u"""Setup new tweak values and internal buffer.
    ...
    def _process_block(self, block, byte_count_add):
        u"""Encrypt internal state using Threefish.
    ...
    def update(self, msg):
        u"""Update internal state with new data to be hashed.
    ...
    def final(self, output=True):
        u"""Return hashed data as bytestring.
        ...
        if not output:
            ...
            while i * 64 < self.digest_size:
                ...
                self.tf.tweak = [0, self.block_type[u'out_final']]
    ...
    def hexdigest(self):
        u"""Return a hexadecimal representation of the hashed data"""
        return binascii.b2a_hex(self.digest())


class Skein512Random(Skein512):
    u"""A Skein-based pseudo-random bytestring generator.
    ...
    def __init__(self, seed=None, queue_size=512):
        Skein512.__init__(self, block_type=u'nonce')
        self.queue = []
    ...
    def next(self):
        if not self.queue:
            self.queue = array.array(u'B', self.getbytes(self.queue_size))
        return self.queue.pop()

if __name__ == u'__main__':
    print Skein512(u'123').hexdigest()
