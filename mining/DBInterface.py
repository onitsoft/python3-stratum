...
import lib.logger
log = lib.logger.get_logger(u'DBInterface')
...
class DBInterface(object):
    ...
    def signal_handler(self, signal, frame):
        log.warning(u"SIGINT Detected, shutting down")
    ...
    def connectDB(self):
        if settings.DATABASE_DRIVER == u"sqlite":
            log.debug(u'DB_Sqlite INIT')
            from . import DB_Sqlite
            return DB_Sqlite.DB_Sqlite()
        elif settings.DATABASE_DRIVER == u"mysql":
            if settings.VARIABLE_DIFF:
                log.debug(u"DB_Mysql_Vardiff INIT")
                from . import DB_Mysql_Vardiff
                return DB_Mysql_Vardiff.DB_Mysql_Vardiff()
            else:
                log.debug(u'DB_Mysql INIT')
                from . import DB_Mysql
                return DB_Mysql.DB_Mysql()
        elif settings.DATABASE_DRIVER == u"postgresql":
            log.debug(u'DB_Postgresql INIT')
            from . import DB_Postgresql
            return DB_Postgresql.DB_Postgresql()
        elif settings.DATABASE_DRIVER == u"none":
            log.debug(u'DB_None INIT')
            from . import DB_None
            return DB_None.DB_None()
        else:
            log.error(u'Invalid DATABASE_DRIVER -- using NONE')
            log.debug(u'DB_None INIT')
            from . import DB_None
            return DB_None.DB_None()
    ...
    def scheduleImport(self):
        ...
        if use_thread:
            self.queueclock = reactor.callLater(
                settings.DB_LOADER_CHECKTIME, self.run_import_thread)
        else:
            self.queueclock = reactor.callLater(
                settings.DB_LOADER_CHECKTIME, self.run_import)

    def run_import_thread(self):
        log.debug(u"run_import_thread current size: %d", self.q.qsize())

        if self.q.qsize() >= settings.DB_LOADER_REC_MIN or time.time(
        ) >= self.next_force_import_time:  # Don't incur thread overhead if we're not going to run
            reactor.callInThread(self.import_thread)

        self.scheduleImport()

    def run_import(self):
        log.debug(u"DBInterface.run_import called")
    ...
    def _update_pool_info(self, data):
        self.dbi.update_pool_info({u'blocks': data[u'blocks'], u'balance': data[u'balance'],
                                   u'connections': data[u'connections'], u'difficulty': data[u'difficulty']})

    def do_import(self, dbi, force):
        log.debug(
            u"DBInterface.do_import called. force: %s, queue size: %s",
            u'yes' if force else u'no',
            self.q.qsize())
        ...
        while self.q.empty() == False and (force == True or self.q.qsize() >=
                                           settings.DB_LOADER_REC_MIN or time.time() >= self.next_force_import_time or forcesize > 0):
            self.next_force_import_time = time.time() + settings.DB_LOADER_FORCE_TIME
            ...
            # try to do the import, if we fail, log the error and put the data
            # back in the queue
            try:
                log.info(u"Inserting %s Share Records", datacnt)
                dbi.import_shares(sqldata)
            except Exception, e:
                log.error(u"Insert Share Records Failed: %s", e.args[0])
    ...
    def found_block(self, data):
        try:
            log.info(u"Updating Found Block Share Record")
            # We can't Update if the record is not there.
            self.do_import(self.dbi, True)
            self.dbi.found_block(data)
        except Exception, e:
            log.error(u"Update Found Block Share Record Failed: %s", e.args[0])

    def check_password(self, username, password):
        if username == u"":
            log.info(u"Rejected worker for blank username")
            return False
        allowed_chars = Set(
            u'0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_-.')
        if Set(username).issubset(allowed_chars) != True:
            log.info(u"Username contains bad arguments")
            return False
        if username.count(u'.') > 1:
            log.info(u"Username contains multiple . ")
            return False
        ...
        if not settings.USERS_CHECK_PASSWORD and self.user_exists(username):
            ...
            return True
        elif settings.USERS_AUTOADD:
            if self.dbi.get_uid(username):
        ...
        log.info(u"Authentication for %s failed" % username)
        return False
