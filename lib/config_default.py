...
LOGFILE = u'stratum.log'  # 'stratum.log'
...
SSL_PRIVKEY = u'server.key'
SSL_CACERT = u'server.crt'
...
# Low value leads to more frequent session reinitializing (like
# downloading address history).
...
# short history will cause that in some edge cases clients won't receive
# older events.
HTTP_BUFFER_LIMIT = 10000

# User agent used in HTTP requests (for both HTTP transports and for proxy
# calls from services)
USER_AGENT = u'Stratum/0.1'

# Provide human-friendly user interface on HTTP transports for browsing
# exposed services.
BROWSER_ENABLE = True
...
COINDAEMON_TRUSTED_HOST = u'127.0.0.1'
COINDAEMON_TRUSTED_PORT = 8332  # RPC port
COINDAEMON_TRUSTED_USER = u'stratum'
...
# For SHA256 PoS Coins which support TX Messages please enter yes in the
# TX sel$
COINDAEMON_ALGO = u'riecoin'
COINDAEMON_Reward = u'POW'
COINDAEMON_SHA256_TX = u'yes'
...
ADMIN_RESTRICT_INTERFACE = u'127.0.0.1'

# Use "./signature.py > signing_key.pem" to generate unique signing key
# for your server
...
IRC_NICK = u"stratum%s"  # Skip IRC registration
...
# Hardcoded list of Stratum nodes for clients to switch when this node is
# not available.
PEERS = [
    {
        u'hostname': u'stratum.bitcoin.cz',
        u'trusted': True,  # This node is trustworthy
        u'weight': -1,  # Higher number means higher priority for selection.
...
# and as high as min(VDIFF_MAX or the coin daemon's difficulty)
# Set the maximum difficulty to the *coin difficulty.
USE_COINDAEMON_DIFF = False
# Update the *coin difficulty once a day for the VARDIFF maximum
...
# Please make sure your front end is compatible with the block hash in the
# solutions table.
# If enabled, send the block hash. If false send the scrypt hash in the
# shares table
